...
export function useAppHotkeys(extraHotkeys: HotkeyHandler[] = []) {
  ...
  const hotkeys = useMemo<HotkeyHandler[]>(() => [
    ...
    },
    {
      ...
      shift: true,
      handler: () => {
        // Ctrl+Shift+S：阶段性手动打标记快照（如"刚写完第三章高潮"）。
        // 与 Ctrl+S 的区别：仅创建带描述的版本快照（autoGenerated=false，在版本历史中突出显示），
        // 不触发 saveProject 落盘——避免与 Ctrl+S 重复落盘。
        // 描述带时间戳便于后续在版本历史中定位，无需用户手动输入（Electron 原生 prompt 会阻塞主进程）。
        // saveVersion 内部 M12 防重复：内容未变时静默跳过，此处用 toast 告知用户结果。
        if (!currentChapterId) {
          toast.info('未选中章节', '请先选中章节再打标记快照');
          return;
        }
        const now = new Date();
        const timeStr = now.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
        const before = useAppStore.getState().versions[currentChapterId]?.length ?? 0;
        saveVersion(currentChapterId, `手动标记 ${timeStr}`);
        const after = useAppStore.getState().versions[currentChapterId]?.length ?? 0;
        if (after > before) {
          toast.success('已创建标记快照', `版本 ${after} · ${timeStr}`);
        } else {
          toast.info('未创建快照', '内容与最近版本相同，已跳过');
        }
      },
      description: '创建标记快照 (Ctrl+Shift+S)',
    },
    ...extraHotkeys,
