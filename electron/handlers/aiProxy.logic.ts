...
export const AI_PROXY_DEFAULT_MODEL_MAP: Record<string, string> = {
  local: 'qwen2.5:7b',
  openai: 'gpt-4o-mini',
...
// 构造请求的入参形状：与 ai:proxyStream / ai:proxyLLM handler 内联变量一致。
export interface AIProxyRequestParams {
  provider: string;
  // baseUrl 可选，覆盖 settings.baseUrl（与 handler 行为一致：params.baseUrl 优先）
  baseUrl?: string;
  // model 可选，覆盖 settings.model（与 handler 行为一致：params.model 优先）
  model?: string;
...
// 主进程存储的 AI 设置子集（仅含 buildAIProxyRequest 需要的字段）。
export interface AIProxyRequestSettings {
  apiKey: string;
  baseUrl?: string;
  model?: string;
}

// buildAIProxyRequest 返回值：handler 据此调用 fetch(url, { method, headers, body, signal })
export interface BuiltAIProxyRequest {
  url: string;
  headers: Record<string, string>;
  body: string;
  // 调试用：解析后的 model（便于测试断言 modelMap 回退逻辑）
  model: string;
}

// 解析 AI provider 响应 JSON 的结果。
// - ok:true 时 content 为非空字符串
// - ok:false 时 error 为错误描述（与 handler 的返回值结构一致）
export type ParsedAIProxyResponse = { ok: true; content: string } | { ok: false; error: string };
...
export function buildAIProxyRequest(
  params: AIProxyRequestParams,
  settings: AIProxyRequestSettings,
): BuiltAIProxyRequest {
...
export function parseAIProxyResponse(data: unknown, provider: string): ParsedAIProxyResponse {
  const d = data as { message?: { content?: unknown }; choices?: Array<{ message?: { content?: unknown } }> };
