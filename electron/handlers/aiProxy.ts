...
import { resolveFilePath } from './storage';
import { buildAIProxyRequest, parseAIProxyResponse } from './aiProxy.logic';

const aiAbortControllers = new Map<string, AbortController>();

// 中止所有进行中的 AI 请求：before-quit 调用，避免 fetch 句柄悬挂
function abortAllAIRequests(): void {
...
function isAllowedAiBaseUrl(baseUrl: string, provider?: string): boolean {
...
// ============================================================================
function registerAISettingsHandlers(): void {
  ...
  safeIpcHandle('ai:saveSettings', async (_event, settings: {
    ...
    temperature: number;
    maxTokens: number;
    // 允许额外字段（style/descriptionDensity 等 UI 偏好），不涉及安全，原样落盘
    [key: string]: unknown;
  }) => {
    try {
      ...
      return await withWriteMutex(filePath, async () => {
        ...
        try {
          await fs.writeFile(tmp, JSON.stringify(storedSettings), 'utf-8');
...
function registerAIProxyHandlers() {
  ...
  safeIpcHandle('ai:abort', (_event, requestId: unknown) => {
...
export { registerAIProxyHandlers, registerAISettingsHandlers, abortAllAIRequests };
