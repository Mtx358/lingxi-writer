...
// ProjectOp 类型：与 storage:patchProjects handler 的 op 入参形状一致。
// project 字段约束为含 string id 的对象（handler 已通过 validateProject 校验）。
export type ProjectOp =
  | { type: 'add'; project: Record<string, unknown> & { id: string } }
  | { type: 'remove'; id: string }
  | { type: 'update'; project: Record<string, unknown> & { id: string } }
  | { type: 'clear' };

// 从未知项目对象中提取 id（与原 handler 的 getId 一致）。
// 返回 null 表示该项目无有效 id（用于 update op 的防御性检查）。
function getProjectId(p: unknown): string | null {
...
export function applyProjectsOps(current: unknown[], ops: ProjectOp[]): unknown[] | null {
  let arr = [...current];
  for (const op of ops) {
    switch (op.type) {
      case 'add': {
      ...
      case 'update': {
        ...
        arr = arr.map(p => {
          if (getProjectId(p) === newId) {
            ...
            return { ...(p as Record<string, unknown>), ...(op.project as Record<string, unknown>) };
          }
          return p;
        });
