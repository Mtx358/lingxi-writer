...
exports.EXPORT_ALLOWED_EXTENSIONS = void 0;
exports.isSafeProjectFilePath = isSafeProjectFilePath;
exports.isSafeBackupPath = isSafeBackupPath;
exports.isSafeExportFilePath = isSafeExportFilePath;
exports.isInsideDataDir = isInsideDataDir;
exports.resolveRealPath = resolveRealPath;
exports.assertRealPathInside = assertRealPathInside;
exports.isSafeIdentifier = isSafeIdentifier;
exports.isValidProjectFileData = isValidProjectFileData;
exports.validateAIProxyParams = validateAIProxyParams;
exports.validateAIProxyLLMParams = validateAIProxyLLMParams;
exports.getAllowedProjectFileRoots = getAllowedProjectFileRoots;
...
function getAllowedProjectFileRoots() {
    ...
    // 去重 + 规范化
    return Array.from(new Set(roots.map(p => node_path_1.default.resolve(p))));
}
...
function isSafeProjectFilePath(filePath) {
    ...
    if (!node_path_1.default.isAbsolute(filePath))
        return false;
    if (!filePath.toLowerCase().endsWith('.cwp'))
        return false;
    const normalized = node_path_1.default.normalize(filePath);
    // 规范化后必须仍以 .cwp 结尾（防止 ../evil.cwp/. 这类绕过）
    if (!normalized.toLowerCase().endsWith('.cwp'))
        return false;
    // 必须位于白名单根目录之一之内
    const allowedRoots = getAllowedProjectFileRoots();
    const isInsideAllowed = allowedRoots.some(root => normalized === root || normalized.startsWith(root + node_path_1.default.sep));
    if (!isInsideAllowed)
        return false;
    return true;
}
...
function isSafeBackupPath(p) {
    ...
    const backupsRoot = (0, shared_1.getBackupsDir)();
    return resolved === backupsRoot || resolved.startsWith(backupsRoot + node_path_1.default.sep);
}
...
exports.EXPORT_ALLOWED_EXTENSIONS = EXPORT_ALLOWED_EXTENSIONS;
...
function isSafeExportFilePath(filePath) {
    ...
    if (!EXPORT_ALLOWED_EXTENSIONS.has(ext))
        return false;
    const normalized = node_path_1.default.normalize(filePath);
    // 规范化后扩展名仍须在白名单中（防止 ../evil.exe/. 这类绕过）
    const normalizedExt = node_path_1.default.extname(normalized.toLowerCase());
    if (!EXPORT_ALLOWED_EXTENSIONS.has(normalizedExt))
        return false;
...
function isInsideDataDir(filePath) {
    ...
    const dataDir = (0, shared_1.getDataDir)();
    return resolved === dataDir || resolved.startsWith(dataDir + node_path_1.default.sep);
}
    ...
    try {
        ...
        if (code === 'ENOENT') {
            ...
            try {
                ...
                return node_path_1.default.join(realParent, base);
            }
            catch {
                return null;
            }
...
async function assertRealPathInside(filePath, allowedRoots) {
    ...
    const realRoots = await Promise.all(allowedRoots.map(async (r) => {
        const rr = await resolveRealPath(r);
        return rr ?? r;
    }));
    return realRoots.some(root => real === root || real.startsWith(root + node_path_1.default.sep));
}
...
function isValidProjectFileData(data) {
    if (!data || typeof data !== 'object')
        return false;
    const d = data;
    if (!d.project || typeof d.project !== 'object' || Array.isArray(d.project))
        return false;
    ...
    if (d.versions != null && (typeof d.versions !== 'object' || Array.isArray(d.versions)))
        return false;
    return true;
}
// ============================================================================
